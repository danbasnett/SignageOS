#!/bin/bash -e
# Runs directly inside the chroot — pi-gen handles the chroot setup
# Install Node.js, NDI SDK, Companion Satellite, web UI deps
...
curl -fsSL "https://github.com/bitfocus/companion-satellite/releases/download/v${SAT_VERSION}/companion-satellite-${PLATFORM}.tar.gz" \
  | tar -xzf - -C /opt/companion-satellite --strip-components=1
...
# ── Sudoers ───────────────────────────────────────────────────────────────────
echo "${FIRST_USER_NAME:-signaeos} ALL=(ALL) NOPASSWD: ALL" \
  > /etc/sudoers.d/010_signaeos-nopasswd
chmod 440 /etc/sudoers.d/010_signaeos-nopasswd
echo "Sudoers configured for ${FIRST_USER_NAME:-signaeos}"

# ── Version ───────────────────────────────────────────────────────────────────
echo "${SIGNAEOS_VERSION:-0.1.0}" > /etc/signaeos-version
